...
class TMDBMovieScraper(object):
    def __init__(self, url_settings, language, certification_country, search_language="",
                 plot_language=None, tagline_language=None, set_language=None,
                 genres_language=None, originaltitle_language=None, artwork_language=None,
                 trailer_language=None, fetch_sets=True):
        ...
        self.plot_language = plot_language or language
        self.tagline_language = tagline_language or language
        self.set_language = set_language or language
        self.genres_language = genres_language or language
        self.originaltitle_language = originaltitle_language or 'Original'
        self.artwork_language = artwork_language or language
        self.trailer_language = trailer_language or language
        self.fetch_sets = fetch_sets
        self._urls = None
    ...
    def _gather_details(self, media_id):
        ...
        # Extra API calls for per-field languages that differ from main language
        extra_lang_movies = {}
        extra_languages = set()
        if self.plot_language != self.language:
            extra_languages.add(self.plot_language)
        if self.tagline_language != self.language:
            extra_languages.add(self.tagline_language)
        if self.genres_language != self.language:
            extra_languages.add(self.genres_language)
        if self.originaltitle_language != 'Original' and self.originaltitle_language != self.language:
            extra_languages.add(self.originaltitle_language)
        for lang in extra_languages:
            resp = tmdbapi.get_movie(media_id, language=lang)
            if resp and not resp.get('error'):
                extra_lang_movies[lang] = resp

        # Collection: use set_language (skip if sets are disabled)
        collection = None
        collection_fallback = None
        if self.fetch_sets and movie['belongs_to_collection']:
            collection = _get_moviecollection(movie['belongs_to_collection'].get('id'), self.set_language)
            collection_fallback = _get_moviecollection(movie['belongs_to_collection'].get('id'))
            if collection and collection_fallback and 'images' in collection_fallback:
                collection['images'] = collection_fallback['images']

        # Trailer: use trailer_language with fallback chain
        trailer = self._fetch_trailer_for_language(media_id, movie, movie_fallback)

        return {'movie': movie, 'movie_fallback': movie_fallback, 'collection': collection,
            'collection_fallback': collection_fallback, 'extra_lang_movies': extra_lang_movies,
            'trailer': trailer}

    def _fetch_trailer_for_language(self, media_id, movie, movie_fallback):
        """Get trailer with the preferred trailer language, falling back to main language then English."""
        # If trailer_language matches main language, trailers are already in movie
        if self.trailer_language == self.language:
            return _parse_trailer(movie.get('trailers', {}), movie_fallback.get('trailers', {}))
        # Fetch with trailer language
        trailer_resp = tmdbapi.get_movie(media_id, language=self.trailer_language,
                                          append_to_response='trailers')
        if trailer_resp and not trailer_resp.get('error'):
            trailer = _parse_trailer(trailer_resp.get('trailers', {}), {})
            if trailer:
                return trailer
        # Fallback to main language
        trailer = _parse_trailer(movie.get('trailers', {}), {})
        if trailer:
            return trailer
        # Fallback to English
        return _parse_trailer(movie_fallback.get('trailers', {}), {})

    def _assemble_details(self, movie, movie_fallback, collection, collection_fallback,
                          extra_lang_movies=None, trailer=None):
        if extra_lang_movies is None:
            extra_lang_movies = {}

        # Pick fields from the per-field language responses, falling back to main, then English
        plot_movie = extra_lang_movies.get(self.plot_language, movie)
        tagline_movie = extra_lang_movies.get(self.tagline_language, movie)
        genres_movie = extra_lang_movies.get(self.genres_language, movie)

        # Determine originaltitle based on language setting
        if self.originaltitle_language == 'Original':
            originaltitle = movie['original_title']
        elif self.originaltitle_language == self.language:
            originaltitle = movie['title']
        else:
            ot_movie = extra_lang_movies.get(self.originaltitle_language)
            originaltitle = ot_movie.get('title', movie['original_title']) if ot_movie else movie['original_title']

        info = {
            'title': movie['title'],
            'originaltitle': originaltitle,
            'plot': plot_movie.get('overview') or movie.get('overview') or movie_fallback.get('overview'),
            'tagline': tagline_movie.get('tagline') or movie.get('tagline') or movie_fallback.get('tagline'),
        ...
        available_art = _parse_artwork(movie, collection, self.urls, self.artwork_language)

        _info = {'set_tmdbid': movie['belongs_to_collection'].get('id')
            if self.fetch_sets and movie['belongs_to_collection'] else None}
